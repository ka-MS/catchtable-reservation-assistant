...
- **`cleanup()` 주석이 새 동작과 정반대였다.** "감싸지 않은 두 호출이
  던지면 이후 cleanup과 flush가 실행되지 않는다"를 그대로 뒀다. 코드만
  고치고 그 위 주석을 안 고친 것이다. 나중에 네 번째 포트 호출을 추가하는
  사람이 abort-on-throw를 의도된 설계로 읽으면 #27이 재발한다. 새 호출은
  반드시 감싸라는 지시로 바꿨다(`AGENTS.md` §6).
- **guard가 예외를 관측 없이 삼켰다.** 미래의 흐름 버그를 잡으라고 둔
  guard인데, 정작 그날이 오면 run은 정상 결과를 반환하고 누수는 조용히
  일어나 추적할 근거가 없었다. 증상은 막고 원인은 은폐하는 모양이다.
  `observe.event("error", ...)` 한 줄을 넣어 계약은 유지하면서 근거를
  남긴다.
...
**한도를 올리지 않고 주석을 줄였다(536자 → 리뷰 반영 후 751자).** 그 검사는 PIN 폐기가 비동기
flush보다 먼저임을 지키는 보안 가드이고, 설명문 때문에 느슨하게 만들 이유가
없다. 긴 설명은 테스트 파일과 이 문서에 있다.

리뷰 반영으로 751자까지 올라갔다. **이 블록에 뭔가 더 넣으려면 한도에
먼저 부딪힌다.** 그때는 한도를 올릴 게 아니라 설명을 문서로 옮긴다.
