...
- **XHR 슬롯 응답 감시** — 분석·설계·구현계획 완료: `docs/specs/xhr-slot-watch/10-analysis-design.md`. 방식은 PerformanceObserver 도착 신호 + 스캔 버스트(폴백으로 기존 사이클 스캔 유지). **다음 액션: 스펙 §2의 정찰 R1~R5(슬롯 XHR URL 패턴, 재요청 유발 조건, 렌더 지연, 격리 월드 가시성, 오픈 전후 응답 차이)를 DevTools로 실측해 site-behavior.md에 기록** — 실측 전 구현 금지.
