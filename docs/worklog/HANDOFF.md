...
- **문서가 코드보다 앞서 있다.** 현재 코드는 여전히 예약 폼 도착 시 `HANDED_OFF`로 종료한다. 아래 두 가지가 **최우선 미완료(blocking)** 항목이다.
  ...
  2. **`availabilityProbeMode` 기본값 리그레션** — `src/sidepanel/form-model.ts`의 신규 폼 기본값이 `empty_exit`로 들어가 있다(커밋 `b0bbbd9`, "초기화 버튼"). 이는 RT-14의 명시적 결정(**기본값 `off` 유지, 동등 비교군으로 성능·요청 증가량 확인 전까지 기본 비활성**)과 충돌한다. 신규 예약 작업이 기본으로 XHR probe를 켠 채 실행되므로 `"off"`로 되돌려야 한다. 문서(HANDOFF·RT-14)의 "기본값 off" 서술이 옳고 코드가 틀린 상태다.
