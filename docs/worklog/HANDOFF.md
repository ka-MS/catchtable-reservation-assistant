...
**갱신:** 2026-07-12
**브랜치:** `main`
**작업 로그:** `docs/worklog/2026-07-12-10-clock-collected-samples.md`
...
- **사용자 조치: 저장된 job 설정의 "시계 표본"을 9로 확인·재저장.** 코드 기본값은 이미 9이나, 이전 실런들은 저장된 값(5)을 썼다. worklog 10에서 표본 부족을 진단하는 metric(`clockCollectedSamples`)도 추가됨 — 다음 실런의 trace 상세에서 "표본 N개 중 M개 사용" 문구 유무로 fetch 실패 여부를 바로 확인 가능.
- **시계 초기/최종 일치 재확인** — 표본 9로 재실런 후 초기·최종 오프셋이 ±150ms 안에서 일치하는지 확인. 일치하면 openDelta가 실런 간 비교 가능해지고, 그다음 XHR 응답 감시로 감지 양자화(+200~300ms)를 제거한다.
