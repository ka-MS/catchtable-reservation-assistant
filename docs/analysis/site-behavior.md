...
### 12.22 성공 완료 문구와 방문예정 표기 변형 `[실측: 스시 호시카이 2026-08-06, 로그인, 실예약]`

§12.21 수정 dist로 스시 호시카이 2026-09-08 오후 6:30, 2명, 예약금
230,000원 → 0원 실행을 완주했다. 폼 판정과 외부 최종 제출은 통과했고
(`form_ready` → `outer_claim` → `outer_dispatch` 각 1회) 실제 예약이
생성됐다. 결과는 `COMPLETED`가 아니라 결과 불명 인계였다.

- URL은 `/ct/mydining/my/planned`로 전이했다(§12.5와 동일). `[실측]`
- **완료 문구가 `예약을 완료했습니다`다.** §12.5·§12.16의
  `자동결제로 예약을 완료했습니다`가 아니다. 진단 fragment 기준 DOM은
  `<div><div><span>예약을 완료했습니다</span></div><p>초대장을 보내 예약
  정보를 공유해 주세요.</p></div>`이고, 가장 작은 element의 정규화
  textContent가 정확히 `예약을 완료했습니다`다. §12.16의
  "가장 작은 visible element" 판정 규칙 자체는 그대로 성립한다. `[실측]`
- 완료 오버레이의 예약 카드는 `09.08(화) · 오후 06:30·2명`이다 —
  24시간제 zero-padding 표기다. `[실측]`
- 초대장 오버레이 구성은 §12.5와 같다(`초대장 편집`/`링크로 공유`/
  `카카오톡으로 공유`). `[실측]`
- **방문예정 목록 항목은 일치했다.** 같은 페이지를 read-only로 재판독한
  결과 visible `li` 하나의 정규화 textContent가
  `D-33예약스시 호시카이제주 제주시 · 스시오마카세2026.09.08 (화)·오후 6:30·2명초대장 보내기`였다.
  매장 표시명·`2026.09.08 (화)`·`2명`이 모두 들어 있다. `[실측]`
- 목록의 시각 표기는 **12시간제 `오후 6:30`** 으로, 같은 예약을 폼 하단
  요약에서는 24시간제 `오후 18:30`(§12.21), 완료 오버레이에서는
  `오후 06:30`으로 쓴다. 같은 예약의 시각 표기가 화면마다 세 가지다.
  `[실측]`
- 따라서 이 실행의 실패 원인은 완료 문구 하나였다. path와 목록은
  일치했다. `[실측]`
- 상단 제목 `h1 마이다이닝`은 `main` 밖 top bar에 있다. 성공 화면
  스냅샷도 예약 폼과 같은 이유로 문서 전체를 범위로 읽어야 한다.
  `[실측]`

`자동결제` 문구는 예약 폼(§12.21)과 완료 화면 양쪽에서 사라졌다.
두 표기 모두 실측된 사실이므로 완료 문구 판정은 두 변형을 함께 받는다.

### 12.23 §12.21·§12.22 변형의 다매장 교차 확인 `[사용자 확인: 3개 매장 2026-08-06, 실예약]`

§12.21·§12.22 대응 dist로 스시 호시카이 외 **다른 매장 3곳**을 실행해
모두 최종 완주(`COMPLETED`)에 성공했다고 사용자가 확인했다.

- 서로 다른 4개 매장에서 suffix 판정(`/예약하기$/`,
  `/예약을 완료했습니다$/`)이 성립한다. §12.21·§12.22의 문구 변형은
  스시 호시카이 고유가 아니다. `[사용자 확인]`
- 개별 매장이 `예약하기`와 `자동결제로 예약하기` 중 어느 라벨을
  썼는지는 수집하지 않았다. 완주 성공만으로는 구분되지 않으므로
  라벨 분포는 여전히 **미실측**이다. `[미실측]`
- 매장명·run id·telemetry 원본이 없는 구두 확인이므로 `[실측]`이
  아니라 `[사용자 확인]`으로 둔다. DOM 단위 사실은 §12.21·§12.22의
  단일 매장 실측이 근거로 남는다.
