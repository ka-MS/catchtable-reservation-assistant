...
### 스캔 루프의 매 반복에는 관측이 없다

**매 반복에서 실행되는 관측 호출은 없다.** 25ms·10ms·5ms polling 루프의
반복 경로(`inspectSlots` → 대기)에는 관측이 끼어 있지 않다.

다만 "루프 안에 관측이 전혀 없다"는 것은 사실이 아니다. 조건부로 실행되는
관측이 두 개 있다.

| 호출 | 조건 | 빈도 |
|---|---|---|
| `traceAvailabilityEmptyExit` (`applyPendingEmptyExit` 내부) | `empty_exit` wake 신호가 대기 중일 때만 | 사이클당 최대 1회 |
| `traceCycle("EMPTY_EARLY_EXIT")` | 조기 종료가 확정됐을 때만 | 사이클당 최대 1회 |

둘 다 실행 직후 루프를 `break` 하거나 `return` 하므로 반복 비용에
누적되지 않는다. `traceCycle`의 나머지 호출은 사이클 종료 시 1회다.

따라서 관측 분리는 핫패스 성능에 중립이다. 유지해야 할 불변식은
"루프 안에 관측이 없다"가 아니라 **"매 반복 경로에 관측이 없다"** 이다.
