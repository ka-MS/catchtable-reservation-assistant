...
코드·자동 검증·비최종 live 검증이 끝나도 RT-10M 표본 전에는 성능 완료로 판정하지 않는다. 당시 문서 상태는 `fallback 보존형 구현 완료, RT-10M 재측정 대기`였다. 이후 RT-10M과 RT-05를 마쳤지만 body wake 성능 이득과 공식 p95는 여전히 미입증이므로, 최종 상태는 `REDUCE 기능·안전 범위 종료, probe 기본 비활성, 성능 판정 후속 측정`이다.
