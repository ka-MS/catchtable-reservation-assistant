...
- 원본 epoch ms와 KST 열은 Excel이 지수 표기 또는 축약 시간 형식으로 바꾸지 않도록 신뢰된 문자열 수식으로 기록한다.
...
- 양주르 실제 CSV 125행을 검산해 여섯 epoch/KST 쌍의 불일치가 0건임을 확인했다. Excel의 자동 서식이 epoch를 지수 표기로, KST를 `58:46.9`처럼 축약하므로 두 열을 Excel-safe text로 보완했다.
